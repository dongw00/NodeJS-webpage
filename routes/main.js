...
module.exports = (app) => {
    ...
    BoardContents.count({deleted:false},function(err, totalCount){
      // db에서 날짜 순으로 데이터들을 가져옴
       ...
       BoardContents.find({deleted:false}).sort({date:-1}).skip(skipSize).limit(limitSize).exec(function(err, pageContents) {
           if(err) throw err;
           res.render('notice/index', {title: "Board", contents: pageContents, pagination: pageNum});
       });
   });
});
  

  app.post("/notice", upload.array('UploadFile'),(req, res) => {
  ...
  app.get("/write", (req, res) => {
    res.render("notice/board/write", { title: "글쓰기" });
  });
  ...
  app.get("/notice/delete", function(req, res) {
    var contentId = req.param("id");
  ...
  app.get('/notice/download/:path', (req, res) => {
